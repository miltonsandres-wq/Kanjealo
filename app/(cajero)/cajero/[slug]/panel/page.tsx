...
export default function CajeroPanelPage() {
  ...
  const volverAEscanear = () => setClienteData(null);
        ...
        {cargandoCliente ? (
          ...
          <>
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-bold text-navy/40 hover:text-coral transition-colors"
            >
              <QrCode className="w-3.5 h-3.5" /> Escanear otro cliente
            </button>
          ...
          <div className="bg-white rounded-2xl p-10 md:p-16 shadow-sm flex flex-col items-center gap-6 text-center">
            <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ backgroundColor: color + "15" }}>
              <QrCode className="w-9 h-9" style={{ color }} />
            </div>
            <div>
              <p className="font-bold text-navy text-lg">Escanea la tarjeta del cliente</p>
              <p className="text-sm text-navy/40 mt-1">Apunta la cámara al código QR para identificarlo y dar el sello.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-sm">
              <button
                onClick={() => setModalScanner(true)}
                className="flex items-center gap-2 text-sm font-bold px-4 py-3.5 rounded-xl justify-center transition-all text-white shadow-md active:scale-95"
                style={{ backgroundColor: color }}
              >
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm font-bold px-4 py-3.5 rounded-xl justify-center transition-all border-2 active:scale-95"
                style={{ borderColor: color, color }}
              >
