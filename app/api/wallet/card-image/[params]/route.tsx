...
export async function GET(_req: Request, { params }: { params: Promise<{ params: string }> }) {
  try {
    const { params: encoded } = await params;
    const decoded = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));

    const sellos    = Math.min(Number(decoded.s ?? 0), 50);
    const requeridos = Math.min(Number(decoded.r ?? 10), 50);
    const color     = `#${String(decoded.c ?? "FF5C3A")}`;
    const nombre    = String(decoded.n ?? "Programa de Lealtad");
    ...
    return new Response("Error generando imagen", { status: 400 });
  }
}
